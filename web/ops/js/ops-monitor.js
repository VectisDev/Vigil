  ...
  try {
    ...
    if (r.ok) {
      ...
      if (res) {
        ...
        // Badge de formato del país (del preset). JSON/CSV = adaptador listo;
        // AUTO = se detecta por contenido en la primera captura.
        const fmt = (d.detected_format || '').toUpperCase();
        let fmtBadge = '';
        if (fmt === 'AUTO') {
          fmtBadge = ` <span class="badge badge-neutral" title="El formato se detecta automáticamente en la primera captura">AUTO</span>`;
        } else if (fmt && fmt !== 'UNKNOWN') {
