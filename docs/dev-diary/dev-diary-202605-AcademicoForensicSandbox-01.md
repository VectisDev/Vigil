...
El análisis forense de CENTINEL vivía enterrado en código Python que requería instalación, ambiente configurado y conocimiento técnico para ejecutar. Un observador electoral, un periodista de datos o un evaluador de NDI no podía ver los 25 detectores en acción sin primero instalar dependencias y entender la estructura del proyecto. En la práctica, el argumento forense no existía para el 95% de las personas que necesitaban verlo: existía solo para quienes lo construyeron.
...
Se construyó `web/academico/` de cero: una simulación adversarial de 30 días y 2880 snapshots con una matriz de inyección A/B/C que modela timing (cuándo aparece la anomalía), magnitude (qué tan grande es el desvío) y frequency (qué tan a menudo ocurre). Las 25 reglas de detección fueron calibradas a los datos reales de HN-2025 con los pesos partidarios de PN, LIBRE y PSH. La interfaz incluye velocidades de playback 1×/10×/100×/500×, toggle de loop, y export a PNG y PDF sin ningún CDN externo. Los colores partidarios son los oficiales verificados.
...
- **Velocidades de playback:** 1× para análisis, 500× para demostrar que el sistema cubre un mes de conteo en segundos.
...
La democratización del argumento forense es tan urgente como el argumento mismo. Un sistema que solo los técnicos pueden entender es un sistema que solo los técnicos pueden defender — y los técnicos no son quienes aprueban grants, escriben las coberturas periodísticas o movilizan a los observadores. La accesibilidad del argumento no es UX: es una condición de impacto.
...
`web/academico/` was built from scratch: an adversarial simulation of 30 days and 2,880 snapshots with an A/B/C injection matrix modeling timing (when the anomaly appears), magnitude (how large the deviation is), and frequency (how often it occurs). The 25 detection rules were calibrated against real HN-2025 data with the partisan weights of PN, LIBRE, and PSH. The interface includes playback speeds of 1×/10×/100×/500×, loop toggle, and PNG/PDF export with no external CDN. Partisan colors are the verified official ones.
...
- **Playback speeds:** 1× for analysis, 500× to demonstrate the system covers a month-long count in seconds.
