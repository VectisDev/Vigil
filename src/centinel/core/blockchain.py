...
Migración / Migration:
    ...
    # from centinel.core.blockchain import publish_hash_to_chain
    # publish_hash_to_chain(hash, config)

    # DESPUÉS (costo cero — Bitcoin via OpenTimestamps):
    from centinel.core.anchoring import anchor_snapshot_chain
...
def publish_hash_to_chain(*args, **kwargs) -> None:  # noqa: ANN002
    """DEPRECATED. Use centinel.core.anchoring.anchor_snapshot_chain() instead."""
    raise NotImplementedError(
        "blockchain.publish_hash_to_chain() is removed (violates Zero Cost principle). "
        "Use centinel.core.anchoring.anchor_snapshot_chain() instead. "
