...
from vigil.federation.compression import compress_payload

logger = logging.getLogger("vigil.federation.gossip")
...
_DNS_BOOTSTRAP_DOMAIN = "peers-{country}.vigil.vectisdev.com"

_HARDCODED_SEEDS: list[str] = [
    # Add well-known bootstrap node URLs here as the network grows, e.g.:
    # "https://node1.vigil.vectisdev.com",
    # "https://node2.vigil.vectisdev.com",
]
...
def _load_or_generate_keypair() -> tuple[str, str, Path]:
    ...
    from vigil.core.custody import (  # local import to avoid circular deps
...
def _sign_payload(payload: NodePayload, key_path: Path) -> str:
    ...
    from vigil.core.custody import sign_snapshot
...
def _verify_payload_sig(payload: NodePayload) -> bool:
    ...
    from vigil.core.custody import verify_snapshot_signature
...
async def _bootstrap_from_dns_over_https(country: str) -> list[str]:
    ...
    Record format: one URL per TXT value at peers-{country}.vigil.vectisdev.com
    """
...
class GossipEngine:
    ...
    def __init__(
        ...
        self._reputation = reputation  # vigil.federation.reputation.ReputationEngine
