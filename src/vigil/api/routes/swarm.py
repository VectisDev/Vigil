...
from vigil.federation.gossip import GossipEngine
...
from vigil.federation.compression import decompress_payload
from vigil.api.rate_limit import RateLimiter, RateLimitConfig

logger = logging.getLogger("vigil.api.swarm")
...
async def get_checkpoint() -> dict:
    ...
    from vigil.federation.gossip import (
...
async def swarm_status() -> dict:
    ...
    if _engine is None:
        ...
        from vigil.federation.gossip import _load_or_generate_keypair, _derive_node_id
        try:
            pub_hex, node_id, _ = _load_or_generate_keypair()
        except Exception:
            pub_hex, node_id = "", "unknown"
...
async def local_broadcast(request: Request) -> dict:
    ...
    from vigil.federation.gossip import FindingPayload, _make_finding_id
    from datetime import datetime, timezone
...
async def get_reputation_report() -> dict:
    ...
    signature = ""
    try:
        from vigil.federation.gossip import _load_or_generate_keypair
