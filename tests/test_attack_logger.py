...
def test_honeypot_logs_requests_via_flask_client(
    ...
    monkeypatch.setattr("vigil.defense.attack_logger.random.choice", lambda _opts: 404)
...
def test_external_summary_uses_anonymized_ip(
    ...
    monkeypatch.setattr("vigil.defense.attack_logger.requests.post", _fake_post)

    class _Target:
        pass

    monkeypatch.setattr("vigil.defense.attack_logger.resolve_outbound_target", lambda *a, **k: _Target())
    monkeypatch.setattr("vigil.defense.attack_logger.pin_dns_resolution", lambda _target: nullcontext())
