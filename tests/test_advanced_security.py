...
from vigil.defense.advanced_security import (
...
def test_adaptive_cpu_ignores_brief_spike(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    manager = AdvancedSecurityManager(cfg)
    monkeypatch.setattr("vigil.defense.advanced_security.psutil.cpu_percent", lambda interval=0.1: 95.0)
    monkeypatch.setattr(
        "vigil.defense.advanced_security.psutil.virtual_memory", lambda: type("vm", (), {"percent": 10.0})()
    )
...
def test_honeypot_encrypts_events_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("vigil.defense.advanced_security.Fernet", FakeFernet)
    monkeypatch.setattr("vigil.defense.advanced_security._HAS_CRYPTOGRAPHY", True)
...
def test_honeypot_encrypt_requires_key(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("vigil.defense.advanced_security._HAS_CRYPTOGRAPHY", True)
    monkeypatch.setattr("vigil.defense.advanced_security.Fernet", lambda key: None)
...
def test_air_gap_is_rate_limited(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("vigil.defense.advanced_security.time.sleep", lambda _s: None)
    monkeypatch.setattr(
...
def test_backup_github_requires_allowlist(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("vigil.defense.advanced_security.subprocess.run", _fake_run)
...
def test_backup_github_resolves_remote_url_before_allowlist(
    ...
    monkeypatch.setattr(
        "vigil.defense.advanced_security.subprocess.check_output",
