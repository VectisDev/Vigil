...
# ── Load existing discovered endpoints (scanner may have already written them) ──
ep_path = Path("config/prod/endpoints.yaml")
ep_existing: dict = {}
if _use_yaml and ep_path.exists():
    ep_existing = yaml.safe_load(ep_path.read_text()) or {}


def _get_discovered_endpoints() -> list:
    """Return endpoints already discovered by wizard_discover_endpoints.py, if any."""
    return ep_existing.get("cne", {}).get("presidential_endpoints", [])


def _discovered_url_for_code(code: str) -> str:
    """Return real discovered URL for a division code, or empty string if not found."""
    for ep in _get_discovered_endpoints():
        ep_code = str(ep.get("department_code", "")).zfill(2)
        if ep_code == code.zfill(2):
            return ep.get("url", "")
    return ""
...
def build_departments():
    ...
    # Prefer real discovered main_url if present
    discovered_main = ep_existing.get("cne", {}).get("main_url", "")
    if discovered_main and not nat_url:
        nat_url = discovered_main
    ...
    for i, (div_name, cne_code) in enumerate(
        ...
        # Prefer real discovered URL; fall back to template
        url = _discovered_url_for_code(cne_code)
        if not url and pattern:
            template_url = pattern.replace("{cne_code}", cne_code)
            # Only use template if it doesn't still contain {division} placeholder
...
if preset:
    # cne_domains — extract ALL unique domains from discovered + national URLs
    all_urls = []
    main_url_for_domain = ep_existing.get("cne", {}).get("main_url", "") or preset.national_url or ""
    if main_url_for_domain:
        all_urls.append(main_url_for_domain)
    for ep in _get_discovered_endpoints():
        if ep.get("url"):
            all_urls.append(ep["url"])
    # Also include template base domain if present and no discovered endpoints
    if not all_urls:
    ...
    if domains:
        cfg["cne_domains"] = list(dict.fromkeys(domains))  # deduplicate, preserve order

    # base_url
    cfg["base_url"] = ep_existing.get("cne", {}).get("main_url", "") or preset.national_url or ""

    # endpoints block
    endpoints = {}
    if cfg["base_url"]:
        endpoints["nacional"] = cfg["base_url"]
    for d in depts:
        if d["code"] != preset.national_cne_code and d.get("url"):
...
ep_cfg = dict(ep_existing)  # start from what scanner may have written

ep_cfg.setdefault("cne", {})
if preset:
    if preset.national_url and not ep_cfg["cne"].get("main_url"):
        ep_cfg["cne"]["main_url"] = preset.national_url

    # Only write presidential_endpoints if scanner hasn't already written real ones
    if not _get_discovered_endpoints():
        ep_cfg["cne"]["presidential_endpoints"] = [
            {"department_code": int(d["code"]), "url": d["url"]}
            for d in depts
            if d["code"] != preset.national_cne_code and d.get("url")
        ]
        print(f"✓ config/prod/endpoints.yaml — {len(ep_cfg['cne']['presidential_endpoints'])} dept endpoints (from template)")
    else:
        print(f"✓ config/prod/endpoints.yaml — {len(_get_discovered_endpoints())} dept endpoints (from scanner, preserved)")
