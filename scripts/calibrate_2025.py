...
def load_snapshots(data_dir: Path) -> list[dict]:
    ...
    pattern = re.compile(r"HN\.PRESIDENTE\.00-TODOS\.000-TODOS (\d{4}-\d{2}-\d{2} \d{2}_\d{2}_\d{2})\.json")
    snapshots = []
...
def detect_null_blank_anomaly(metrics: list[dict], warning_pct: float = 8.0, critical_pct: float = 12.0) -> list[dict]:
...
def detect_processing_speed(metrics: list[dict], max_actas_per_15min: int = 500) -> list[dict]:
