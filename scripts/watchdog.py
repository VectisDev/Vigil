...
_vital_logger = logging.getLogger("centinel.vital_signs")
...
def load_vital_signs_config(env: str = "prod") -> Dict[str, Any]:
    ...
    try:
        ...
        _vital_logger.warning("vital_signs_config_fallback | usando defaults: %s", exc)
        loaded = {}
...
def _emit_alert(mode: Any, metrics: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if mode == ResilienceMode.NORMAL:
        return
    _vital_logger.warning("vital_signs_alert | mode=%s metrics=%s", mode, metrics)
...
def predict_mode(config: Dict[str, Any], status: Dict[str, Any]) -> ResilienceMode:
