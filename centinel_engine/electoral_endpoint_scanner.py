"""English: Auto-discovery and self-healing module for electoral endpoints — country-agnostic.
Español: Módulo de autodescubrimiento y autocuración para endpoints electorales — agnóstico al país.
"""
...
class ElectoralEndpointScanner:
    ...
    def __init__(
        ...
        # Country-specific scanner parameters — fall back to HN constants when no preset
        if country_preset is not None:
            self.expected_regions = [
                ElectoralEndpointScanner._normalize_token(d) for d in country_preset.divisions
            ]
            self.region_code_map = {
                c: ElectoralEndpointScanner._normalize_token(n)
                for c, n in country_preset.build_cne_map().items()
            }
            self.hint_keys = country_preset.scanner_hint_keys
            self.hint_values = country_preset.scanner_hint_values
        else:
            self.expected_regions = EXPECTED_DEPARTMENTS
            self.region_code_map = DEPARTMENT_CODE_MAP
            self.hint_keys = PRESIDENTIAL_HINT_KEYS
            self.hint_values = PRESIDENTIAL_HINT_VALUES

        self.session = requests.Session()
        self.session.headers.update(DEFAULT_HEADERS)
        self.logger = logging.getLogger(f"electoral_endpoint_scanner.{self.env_name}")
    ...
    def detect_schema(self, sample_payloads: list[Any]) -> dict:
        """Inspect validated JSON payloads and return a detected field_map for normalize_snapshot()."""
        votes_candidates = ["votos", "total_votos", "votosTotales", "votes", "total_votes", "num_votos"]
        candidate_roots = ["candidatos", "candidates", "candidature", "listas", "partidos", "resultados"]
        turnout_keys = ["participacion", "participation", "turnout", "porcentaje_participacion"]

        field_map: dict = {}
        for payload in sample_payloads:
            keys, _ = self._collect_schema_tokens(payload)
            for k in votes_candidates:
                if k in keys:
                    field_map.setdefault("total_votes", []).append(k)
            for k in candidate_roots:
                if k in keys:
                    field_map.setdefault("candidate_roots", []).append(k)
            for k in turnout_keys:
                if k in keys:
                    field_map.setdefault("turnout_pct", []).append(k)
        # Deduplicate
        return {k: list(dict.fromkeys(v)) for k, v in field_map.items()}

    def detect_html_table(self, url: str) -> dict | None:
        """Use Playwright to render url and look for a results table. Returns {table_selector, column_map} or None."""
        try:
            from playwright.sync_api import sync_playwright
        except ImportError:
            self.logger.error("Playwright not installed — cannot detect HTML tables")
            return None
        ...
        try:
            with sync_playwright() as pw:
                ...
                # Look for tables with electoral-looking headers
                ...
                for table in tables:
                    ...
                    try:
                        ...
                        continue
                    electoral_terms = {"CANDIDATO", "VOTOS", "PARTIDO", "VOTO", "RESULTADO", "CANDIDATE"}
                    if any(term in header_text for term in electoral_terms):
                        # Build column_map from th elements
                        headers = [th.inner_text().strip().lower() for th in table.query_selector_all("th")]
                        column_map = {}
                        for i, h in enumerate(headers):
                            if any(t in h for t in ["candidato", "candidate", "nombre", "name"]):
                                column_map["candidate_name"] = i
                            elif any(t in h for t in ["voto", "vote", "total"]):
                                column_map["votes"] = i
                            elif any(t in h for t in ["partido", "party", "sigla"]):
                                column_map["party"] = i
                            elif any(t in h for t in ["porcent", "percent", "%"]):
                                column_map["pct"] = i
                        if "candidate_name" in column_map and "votes" in column_map:
                            result = {"table_selector": "table", "column_map": column_map}
                            break
                browser.close()
        except Exception as exc:
            self.logger.error("detect_html_table failed for %s: %s", url, exc)
    ...
    def _validate_candidates(self, candidates: list[str]) -> tuple[EndpointRecord | None, dict[str, EndpointRecord]]:
        ...
        for candidate in candidates:
            ...
            if department and department in self.expected_regions and department not in per_department:
    ...
    def _index_existing_endpoints(
        ...
        for item in raw_endpoints:
            ...
            if record.level.upper() == "NACIONAL" and national is None:
                ...
                continue
            if record.department and record.department in self.expected_regions and record.department not in per_department:
    ...
    def _build_resilient_endpoint_set(
        ...
        for department in self.expected_regions:
    ...
    def _collect_schema_tokens(self, payload: Any) -> tuple[set[str], list[str]]:
        ...
        def _walk(node: Any) -> None:
            if isinstance(node, dict):
                for key, value in node.items():
                    keys.add(self._normalize_token(str(key)).lower())
                    _walk(value)
            elif isinstance(node, list):
                for item in node:
                    _walk(item)
            else:
                values.append(self._normalize_token(str(node)))

        _walk(payload)
        return keys, values

    def _looks_presidential(self, payload: Any) -> bool:
        ...
        keys, values = self._collect_schema_tokens(payload)
        overlap = len(self.hint_keys.intersection(keys))
        values_corpus = " ".join(values)
        has_presidential_hint = any(token.upper() in values_corpus for token in self.hint_values)
    ...
    def _infer_level(self, url: str, keys: set[str], values: list[str]) -> str:
    ...
    def _infer_department(self, url: str, values: list[str]) -> str | None:
        ...
        if code_match:
            ...
            dept = self.region_code_map.get(code)
            if dept:
                return dept

        # Fallback: name-based inference from URL tokens and payload values.
        corpus = f"{self._normalize_token(url)} {' '.join(values)}"
        for department in self.expected_regions:
            if department in corpus:
                return department
        return None

    def _extract_payload_department(self, payload: Any) -> str | None:
        ...
        def _search(node: Any) -> str | None:
            if isinstance(node, dict):
                for key, value in node.items():
                    if self._normalize_token(str(key)).lower() in DEPT_FIELD_KEYS:
                        if isinstance(value, str):
                            normalized = self._normalize_token(value)
                            for dept in self.expected_regions:
...
def run_endpoint_healer_for_env(env: str) -> dict[str, Any]:
    ...
    healer = ElectoralEndpointScanner(
