"""Auto-discovery and self-healing module for electoral authority endpoints.

Works with any country configured in src/centinel/countries.py.
The active country is read from the CENTINEL_COUNTRY environment variable
(default: "HN" for backward compatibility with the Honduras 2025 pilot).
"""
...
def _build_country_departments() -> tuple[list[str], dict[str, str]]:
    """Load expected departments and code map from the active country preset.

    Falls back to Honduras (HN) if CENTINEL_COUNTRY is unset, preserving
    backward compatibility with the Honduras 2025 pilot deployment.
    """
    try:
        import sys
        ...
        preset = get_country_preset(country_code)

        def _norm(name: str) -> str:
            nfkd = unicodedata.normalize("NFKD", name)
            return nfkd.encode("ascii", "ignore").decode("ascii").upper()

        expected = [_norm(d) for d in preset.divisions]
        code_map: dict[str, str] = {"00": "NACIONAL"}
        for i, div_name in enumerate(preset.divisions):
            code = preset.division_cne_codes[i] if preset.division_cne_codes else f"{i + 1:02d}"
            code_map[code] = _norm(div_name)
        return expected, code_map
    except Exception:
        # Hardcoded HN fallback — only reached if countries.py is unavailable
        _hn_depts = [
        ...
        _hn_codes = {
            "00": "NACIONAL", "01": "ATLANTIDA", "02": "CHOLUTECA", "03": "COLON",
            "04": "COMAYAGUA", "05": "COPAN", "06": "CORTES", "07": "EL PARAISO",
            "08": "FRANCISCO MORAZAN", "09": "GRACIAS A DIOS", "10": "INTIBUCA",
            "11": "ISLAS DE LA BAHIA", "12": "LA PAZ", "13": "LEMPIRA",
            "14": "OCOTEPEQUE", "15": "OLANCHO", "16": "SANTA BARBARA",
            "17": "VALLE", "18": "YORO",
        }
        return _hn_depts, _hn_codes


EXPECTED_DEPARTMENTS, DEPARTMENT_CODE_MAP = _build_country_departments()
...
class ElectoralAuthorityHealer:
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(f"electoral_authority_healer.{self.env_name}")
        ...
        def _walk(node: Any) -> None:
            if isinstance(node, dict):
                for key, value in node.items():
                    keys.add(ElectoralAuthorityHealer._normalize_token(str(key)).lower())
                    _walk(value)
            elif isinstance(node, list):
                for item in node:
                    _walk(item)
            else:
                values.append(ElectoralAuthorityHealer._normalize_token(str(node)))
    ...
    def _looks_presidential(payload: Any) -> bool:
        ...
        keys, values = ElectoralAuthorityHealer._collect_schema_tokens(payload)
    ...
    def _infer_department(url: str, values: list[str]) -> str | None:
        ...
        corpus = f"{ElectoralAuthorityHealer._normalize_token(url)} {' '.join(values)}"
        for department in EXPECTED_DEPARTMENTS:
    ...
    def _extract_payload_department(payload: Any) -> str | None:
        ...
        def _search(node: Any) -> str | None:
            if isinstance(node, dict):
                for key, value in node.items():
                    if ElectoralAuthorityHealer._normalize_token(str(key)).lower() in DEPT_FIELD_KEYS:
                        if isinstance(value, str):
                            normalized = ElectoralAuthorityHealer._normalize_token(value)
...
def run_endpoint_healer_for_env(env: str) -> dict[str, Any]:
    ...
    healer = ElectoralAuthorityHealer(
...
# Backward-compatibility alias — existing code using CNEEndpointHealer continues to work.
CNEEndpointHealer = ElectoralAuthorityHealer
