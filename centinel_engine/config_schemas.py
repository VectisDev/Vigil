...
Centralizes schema definitions for known config files so misconfigurations
...
class ProxyConfigSchema(BaseModel):
    """Schema for `config/<env>/proxies.yaml`."""
...
class CNEEndpointSchema(BaseModel):
    """Nested schema for `endpoints.yaml::cne`."""
...
class HealingSchema(BaseModel):
    """Nested schema for `endpoints.yaml::healing`."""
...
class EndpointsConfigSchema(BaseModel):
    """Schema for `config/<env>/endpoints.yaml`."""

    cne: CNEEndpointSchema
    healing: HealingSchema = Field(default_factory=HealingSchema)


_SCHEMA_REGISTRY: Dict[str, type[BaseModel]] = {
    "proxies.yaml": ProxyConfigSchema,
    "endpoints.yaml": EndpointsConfigSchema,
}
